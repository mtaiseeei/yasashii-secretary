...
description: >
  Microsoft 365（Outlook のメール・予定表、OneDrive、Teams）を秘書につなぐ案内。Claude の設定画面から
  公式コネクタで接続する。「Microsoft につなぎたい」「Outlook／Teams を見て」等で呼び出す。
---
...
> Microsoft 365 がつながりました。Outlook のメール・予定表などを秘書が見られるようになりました。
> つながっているか、直近の予定を1件だけ読んで確認済みです（中身はローカルに保存していません）。
> 次は「今日やること」や「接続の調子を見て」と話しかけてください。
