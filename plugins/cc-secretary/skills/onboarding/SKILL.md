...
name: onboarding
description: >
  秘書の初回セットアップ。やさしい数問だけ伺って、秘書の家（secretary/）を作り、
  git で最初の区切りを記録する。初めて /secretary を呼んだときに使う。
---

# オンボーディング（初回セットアップ）

初めての人に、やさしい数問だけを伺い、その回答で「秘書の家」＝ `secretary/` フォルダを作ります。
最後に git（作業の履歴を残す仕組み）で最初の区切りを記録します。**インターネットには送りません**（ローカルのみ）。
...
> まず3つだけ伺います。そのあと「秘書の家」フォルダを用意し、最初の区切りを記録します（数十秒で終わります）。
...
「いまは『道具』の段階です。秘書の家を用意しています」と一言添えてから、次を行う。
...
1. `secretary/` フォルダの中で `git init`（履歴を残す準備）する。
   - すでに親フォルダが git 管理下でも、秘書の家は `secretary/` 単独で `git init` する。
2. `secretary/` の中身をすべて追加して、日本語のメッセージで**最初のコミット**（作業の区切りの記録）を作る。
   - コミットメッセージ例: `秘書の家を作成（初回セットアップ）`
3. **push はしない**。リモート（インターネット上の保管先）の設定も、送信もしない。

英語のエラーが出たら、そのまま見せず「何が起きて・どうすれば直るか」に言い換えてから伝える
...
> 秘書の家ができました。いまいるフォルダの中の「secretary」に、記憶と成果物の置き場を用意しました。
