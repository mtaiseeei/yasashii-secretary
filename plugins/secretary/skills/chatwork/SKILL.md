---
name: chatwork
description: Chatworkの初回接続、ルーム・自動取得の間隔の設定変更、取得状態の確認、保存済み履歴の検索、確認付き手動取得を行う。ユーザーが「Chatworkにつなぎたい」「Chatworkで探して」、Claude Codeで「/chatwork」、Codexで「$chatwork」と依頼したときに使う。
---
...
このSkillは、現在の依頼でChatworkの接続・設定・検索が明示された場合だけ使う。Project Clarityの閲覧、Item作成、
Attention、Hookから自動取得、network、設定変更を行わない。Clarityから参照する場合も既存の確認境界を省略しない。
...
個人設定が必要な応答だけ `secretary/memory/preferences.md` の該当節を読む。通常報告の項目数、prefix、Markdown構造をここで再定義せず、すべての
...
2. Tokenの値を尋ねず、ローカルwizardをloopbackで起動する。
   ...
   `node "${SECRETARY_PLUGIN_ROOT}/skills/chatwork/scripts/wizard-server.mjs" --root .`
...
`node "${SECRETARY_PLUGIN_ROOT}/skills/chatwork/scripts/search-flow.mjs" --root . --query "<キーワード>" [--room "<ルーム名またはルームID>"] [--account "<発言者>"] [--from YYYY-MM-DD] [--to YYYY-MM-DD]`
...
- wizard server: `${SECRETARY_PLUGIN_ROOT}/skills/chatwork/scripts/wizard-server.mjs`
- search: `${SECRETARY_PLUGIN_ROOT}/skills/chatwork/scripts/search.mjs`
- search flow: `${SECRETARY_PLUGIN_ROOT}/skills/chatwork/scripts/search-flow.mjs`
