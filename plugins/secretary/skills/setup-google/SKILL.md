...
このSkillはGoogle接続が現在の依頼で明示された場合だけ使う。Project Clarityの閲覧、Item作成、Attention、Hookは
接続、認証、networkの許可ではなく、自動でこのSkillを起動しない。
...
現在のhostで利用可能な公式App／connectorの一覧または実際に呼び出せるtoolを確認し、Gmail、Google Calendar、
Google Driveのうち利用するサービスが存在する場合だけ、そのhostが表示する認可手順へ進む。

- connectorが利用可能なら、現在のhostが出す認可画面でGoogleアカウントと権限を確認し、ユーザー本人の許可後だけ接続する。
...
|---|---|
| まだ許可していない（not authorized / permission） | 「接続の許可がまだのようです。現在のhostのGoogle App／connectorで、表示されている認可内容を確認してください。」 |
| 別アカウントでログインした（wrong account） | 「別の Google アカウントでログインしているかもしれません。使いたいアカウントでログインし直すと直ります。」 |
| 期限切れ・つなぎ直しが必要（expired / reconnect） | 「接続の有効期限が切れたようです。現在のhostに表示されたGoogle App／connectorからつなぎ直してください。」 |
| 接続そのものが無い（not connected / no connector） | 「現在のhostではGoogle接続を確認できませんでした。未確認のまま止め、別hostの手順は流用しません。」 |
