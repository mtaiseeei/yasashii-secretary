...
description: >
  Microsoft 365（Outlook・OneDrive・Teams）の接続・再認証を依頼されたときの案内。
  現在のhostが提供する公式コネクタ／Appを使う。「Microsoftにつなぎたい」「Microsoftの認証をやり直したい」等で呼び出す。
---
...
このSkillはMicrosoft接続が現在の依頼で明示された場合だけ使う。Project Clarityの閲覧、Item作成、Attention、Hookは
...
まず現在のhostで公式Microsoft connector／Appが利用可能かをread-onlyで確かめる。設定対象がまだ確認できない段階では
