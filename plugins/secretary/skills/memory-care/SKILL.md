...
質問は絶対日付と絞り込みへ解釈する。必要な範囲の原本が、canonical root配下から安全に取得済みで、
active / archiveの対象、日付、種類、出典が確認できているなら、`timeline`を追加実行せず、その原本を
LLMが問いに合う順へ整理してよい。これは意味の判断をLLMに任せるための読み取り経路であり、決定的helperを
置き換えるものではない。日付・種類・本文の意味・出典・訂正／変更履歴を保ち、`all`相当ではdecision正本を
優先してjournalの対応する`decided`を二重表示しない。安全に読み取れた範囲と対象外・取得失敗は区別し、
一部の原本から期間全体の網羅、0件、最新状態とは言わない。

原本がまだ安全に揃っていない、対象期間が広い、再現可能な一覧や明示的なtimeline出力が必要な場合だけ、
下記の`timeline`を任意のread-only helperとして使う。helperを使った場合も同じ意味保持と重複抑止を適用する。
canonical root、symlink、active / archive、日付範囲の安全境界で拒否された対象はそこで止め、直接Readで迂回しない。
...
- 取得対象を安全に確認したうえで本当に0件なら「該当する記録はありません」と伝え、期間・種類・キーワードの見直しを案内する。
- timeline相当の閲覧だけではファイルを作らない。ユーザーが「保存して」と明示した場合だけ、ルーターの
