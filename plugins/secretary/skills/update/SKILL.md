...
description: >
  ...
  「最新版にして」「更新ある？」「更新を再開」「バージョンを確認して」「自動更新はどうする？」、
  Claude Codeで「/update」、Codexで「$update」と依頼したときに使う。
---
...
最初に `${SECRETARY_PLUGIN_ROOT}/rules/plain-language.md` を、同じplugin実体・workspaceで該当fileが未変更ならsessionで一度だけ読みます。plugin、workspace、または該当fileが変わった場合だけ、そのfileを再読します。
個人設定が必要な応答だけ `secretary/memory/preferences.md` の該当節を読みます。その後、作業中フォルダをworkspaceとして次を実行します。
