...
function existingBotIdentity(workflow) {
  if (!workflow) return {};
  const name = workflow.match(/git config user\.name "([-A-Za-z0-9_.\[\]]+)"/)?.[1];
  const email = workflow.match(/git config user\.email "([-A-Za-z0-9_.@\[\]]+)"/)?.[1];
  return name && email ? { botName: name, botEmail: email } : {};
}
...
export async function applyGoogleChatConfig({ root, selectedSpaces, availableSpaces = selectedSpaces, interval, automaticPushConsent, commitPushConsent }) {
  ...
  try {
    ...
    writeAtomic(root, entries[2][0], renderGoogleChatWorkflow(interval, scheduleEnabled, existingBotIdentity(snapshots.get(entries[2][0]))));
