...
- 「覚えて」と対象が現在の依頼で明示された低リスクmemory操作は、user-visible scope `memory`だけで許可済みとし、
  ...
  PJ固有であることが明示済みの判断は`project-tools.mjs add-decision`を使い、一般memoryへ二重保存しない。
- 「〜にしよう」「覚えといたほうがいいかも」等の自発候補・request hedgeは副作用0で1問だけ確認する。
  伝聞・推量・留保・否定・条件・訂正は明示保存がある場合のcontent属性として保持する。
  依頼語の引用、現在依頼でない仮定、取り消し、過去照会はwrite 0件とする。
