...
- Git由来のチャット取得、普段の会話、日報や週報の整理を、自然な依頼のまま安全に使いたい方。

### 変わること

- Git checkoutは共通helperで検証し、6つのチャット取得経路で同じ安全境界を使います。
- 通常会話の一人称は既定で「私」とし、本人としての応答を保ちます。秘書名は、初回設定完了・名前質問・rename直後・別repo初回routingの4場面だけで使います。
- 取得済みの安全な情報は、固定scriptを必須にせずLLMが日報・週報・project状況へ整理できます。書込み、削除、Git変更、Clarity確定は従来どおり確認を維持します。
- Project Clarityでは、Attentionと決定・実行の現在状態を安全に整理し、確定操作の確認境界を保ちます。

### 設定・ファイルへの影響

- `言葉遣い.一人称`で一人称を変更できます。既存の記憶、project、identity、チャット履歴は自動変更しません。
- やさしいハーネスのinstall ID `harness@yasashii-harness`、Xmind既定OFF、17 Skills／62 behaviorsを維持します。
...
- Yasashii Secretary固有の案内、release履歴、既存fixtureは維持します。
...
- 『これ覚えて』と頼んだあと、同じ内容をもう一度確認されず、そのまま記録してほしい方。

### 変わること

- 危険のない明示的な記憶依頼は、その依頼を一度だけ保存する許可として扱い、同じturnで記録まで進めます。
- 推測、伝聞、条件付きの内容は、確定事項に言い換えず、その意味を保ったまま記録します。
- 引用、仮定の話、取り消し、過去の記録についての質問は、新しい記憶として保存しません。
- 書き込み途中の失敗、訂正、重複、local checkpoint失敗を区別し、保存できた範囲を正確に伝えます。

### 設定・ファイルへの影響

- 既存の記憶、プロジェクト、identity、チャット履歴を自動で変更しません。
- 記憶の削除、外部送信・公開、Secret、権限変更、一括処理などは、これまでどおり別の確認が必要です。
...
- `0.10.1`のidentity migrationと、それ以前のrelease記録、fixture、tagは履歴として維持します。
