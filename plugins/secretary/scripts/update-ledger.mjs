...
import { assertWorkspaceEdition, loadEditionConfig } from "./lib/edition-guard.mjs";
...
function initialize(args) {
  ...
  try {
    ...
    editionConfig = loadEditionConfig(pluginRoot);
    const editionState = assertWorkspaceEdition(workspace, editionConfig, "update");
    if (editionState.state !== "same-edition") throw new Error("新規台帳は、onboardingでneutral markerを作成した同一edition workspaceにだけ作成できます。");
  } catch (error) {
    fail(error.message, EXIT_REFUSED);
  }
  const ledgerPath = editionConfig.update.ledgerPath;
  const markerPath = editionConfig.workspaceProtection.canonicalMarker;
  const ledger = safeWritePath(workspace, ledgerPath);
  const marker = safeWritePath(workspace, markerPath);
  if (existsSync(ledger)) fail("既存の最小台帳は上書きしません。", EXIT_REFUSED);
  if (!existsSync(marker)) fail("workspace edition markerを確認できないため台帳を作成しません。", EXIT_REFUSED);
  ...
  try {
    const ledgerValue = { schemaVersion: 2, edition: editionConfig.edition, records };
    writeFileAtomicSafe(workspace, ledger, `${JSON.stringify(ledgerValue, null, 2)}\n`, { encoding: "utf8" });
  } catch {
    try { if (existsSync(ledger)) removeSafe(workspace, ledger); } catch { /* 元がnewなので作成物だけを回収 */ }
    fail("最小台帳を安全に作成できませんでした。既存workspaceの診断では作り直しません。", EXIT_REFUSED);
  }
  process.stdout.write(`新規workspaceのedition markerを確認し、最小台帳を作成しました: ${markerPath}, ${ledgerPath}（edition=${editionConfig.edition} / 管理対象${records.length}件）\n`);
}
