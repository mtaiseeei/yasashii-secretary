...
import { entryDecision, inspectWorkspaceEdition, loadEditionConfig } from "./lib/edition-guard.mjs";
...
function marketplaceVersion(data, pluginName) {
  const entries = Array.isArray(data?.plugins) ? data.plugins : [];
  const matches = entries.filter((item) => item?.name === pluginName);
...
export async function latestRelease(args, { fetchImpl = fetch, config = null } = {}) {
  let manifest;
  let changelog;
  let source;
  ...
  if (manifestPath || changelogPath) {
    ...
    try {
      ...
      const marketplaceUrl = process.env.YASASHII_MARKETPLACE_URL || config?.distribution?.marketplaceUrl;
      const changelogUrl = process.env.YASASHII_CHANGELOG_URL || config?.distribution?.changelogUrl;
      if (!marketplaceUrl || !changelogUrl) throw new Error("EditionConfig distribution URL is missing");
...
function workspaceState(workspace, config) {
  const canonicalPath = resolve(workspace, config.update.ledgerPath);
  const legacyPath = config.update.legacyLedgerPaths.map((path) => resolve(workspace, path)).find((path) => existsSync(path));
  const ledgerPath = existsSync(canonicalPath) ? canonicalPath : legacyPath ?? canonicalPath;
  ...
  try {
    ...
    records = Array.isArray(value) ? value : value?.schemaVersion === 2 && value?.edition === config.edition ? value.records : null;
  } catch {
...
function notNewerReason(status, currentVersion, latestVersion) {
  if (status === "same") return `現在版と候補版はどちらも${currentVersion}です。すでに最新版のため更新は開始しません。`;
  if (status === "downgrade-blocked") return `現在版${currentVersion}より候補版${latestVersion}が古いため、downgradeは開始しません。`;
  return null;
}
...
function renderText(result) {
  ...
  return [
    "更新の確認（読み取り専用）",
    `workspace edition: ${result.workspaceEdition.state}`,
    `検出根拠: ${result.workspaceEdition.evidence.join(", ") || "marker／ledgerなし"}`,
    ...(result.workspaceEdition.plannedMigration ? [`予定migration: ${result.workspaceEdition.plannedMigration}`] : []),
    ...(result.workspaceEdition.state === "new"
      ? ["案内: まだ導入前のworkspaceです。実更新ではなくonboardingから開始してください。"] : []),
    ...(["opposite-edition", "mixed", "unknown"].includes(result.workspaceEdition.state)
      ? [`安全停止: ${result.workspaceEdition.state} のため、実更新・migration・ledger／marker変更は行いません。`] : []),
...
async function main() {
  ...
  let editionConfig;
  try { editionConfig = loadEditionConfig(pluginRoot); }
  catch (error) {
    process.stderr.write(`${error.message} 診断以外の操作は行っていません。\n`);
    process.exit(3);
  }
  ...
  const workspaceEdition = inspectWorkspaceEdition(workspace, editionConfig);
  const editionDecision = entryDecision(workspaceEdition, "diagnose");
  const latest = await latestRelease(args, { config: editionConfig });
  const workspaceResult = workspaceState(workspace, editionConfig);
  const updateBlocked = !["same-edition", "legacy-yasashii"].includes(workspaceEdition.state);
  const status = resultStatus(current, latest);
  const updateAllowed = !updateBlocked && status === "update-available";
  const blockedReason = updateBlocked
    ? `${workspaceEdition.state} のため安全に停止します。`
  ...
  const result = {
    ...
    workspaceEdition: { ...workspaceEdition, ...editionDecision },
    ...
    choices: [
      { id: "check-only", label: "今回は確認だけ", available: true },
      {
        id: "proceed-update",
        label: "実更新へ進む",
        available: updateAllowed,
        reason: updateAllowed
          ? "この診断では変更せず、別の最終確認へ進みます。"
          : blockedReason ?? "安全に更新可能な新版を確認できないため停止します。",
      },
    ],
  };
