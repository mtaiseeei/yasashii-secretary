...
- C2・C5・C6・C9・C10・C11・C12 は5/5必須。対象Sprintの保証範囲で、1件でも構文欠陥、secret露出、安全違反、新規回帰、現行面の配布チャネル依存、無確認の更新副作用、Google ChatのOAuth／選択スペース境界違反、または配布前gateの未達があれば不合格。
...
36. **focus・操作領域・文書整合**: running wizardをkeyboardだけで全遷移し、各遷移／非同期結果後のactive element、入力中のfocus保持、主要操作の44px相当hit areaをdesktop／mobile／200%で記録する。`.mcp.json`、onboarding、README、公開ガイドを現行機能・0.7.0・対応サービスと照合する。
37. **0.7.0正式live gate**: F36〜F42、master offline／online、archive gateの合格後、同一release candidateを専用private test workspaceで評価する。Chatwork／Google Chat／OAuth／Repository Secret／Actions／commit・push／pull後search found／同条件再実行を両方実行し、schedule、全Secret、room／space選択、Google OAuth grant／tokenの後始末まで確認する。片方だけ、過去run、合成fixture、後始末未完了は不合格。
...
| C12 | 0.7.0配布準備 | 全監査指摘、version・更新、portable回帰、両チャットlive gate、後始末 | **5** |
...
- 5: HighからLowまでの監査指摘が、定義された保証境界の専用回帰で0件、全配布面が0.7.0、0.6.0から安全更新・両面rollback可能、master suiteとGit archive相当がPASS、同一release candidateでChatwork／Google Chatのlive gateと全後始末が成立する。
- 4以下: 通常フローの値露出、製品管理対象／初回publish inventoryの合理的なsecret検査漏れ、既存stage混入、symlink越境、OAuth再入、CSRF／Origin／Content-Type不備、markerによる履歴欠落、古いrun採用、timeoutなし、plugin未復元、suite未実行、archive不動作、focus／44px欠陥、古い文書、版不一致、片方のlive gate未実施、後始末未完了のいずれかが1件でもある。意図的難読化の未検出だけはsecret検査漏れに数えない。→不合格。
