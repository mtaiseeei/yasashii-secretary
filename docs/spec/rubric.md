...
Claude Code／Codexプラグイン（共通のMarkdown skills、templates、rules、安全なシームとホスト別の正式manifest／marketplace）、一般PJのライト→フル運用、別repo `yasashii-harness` と開発PJ正本への参照導線、
...
6. **リポジトリ境界**: `<workspace-root>/agentic-harness` をコマンド対象・参照元・複製元にせず、編集、checkout、commit、branch、remote変更、生成物作成を行っていないことを実装経路と作業ログから確認する。`yasashii-secretary` に同梱ハーネスが無く、`yasashii-harness` はpublic・`fork=false`の独立downstreamで、GitHubのorigin/upstream remote、fb9c303到達性、yasashii見出しoverlay、宣言的metadata allowlistが成立することを確認する。
...
40. **candidate identityと履歴保護**: candidate／latest／Claude・Codex両marketplace／両plugin manifest／正本・旧raw CHANGELOG／ledger／migration／公開ガイドが `0.8.0` で整合し、公開済み `0.7.0` のmanifest、migration、fixture、評価記録、Git履歴が不変であることを確認する。checkout専用のGit／監査evidence検査と `.git`／監査evidenceを含まないarchive配布検査を分け、同じ配布対象bytesについて両方を0 FAILで完走する。
...
43. **Codex正式plugin配布**: `plugins/secretary/.codex-plugin/plugin.json` の構文、name／version／description、`skills=./skills/`、repo rootの `.agents/plugins/marketplace.json`、`source.path=./plugins/secretary`、policy／category、Claude marketplaceとの共存を検査する。合成 `CODEX_HOME` と専用test workspaceでmarketplace add、plugin addまたは `/plugins`、正本15 skillの一意discoverを回帰確認する。実機smokeでは新しいsession／chat、`$secretary` 等の明示呼出または自然言語trigger、会話8面、wizard導線、更新／再導入後の反映を確認する。`4670438` の同一bytesで既取得のCodex App／CLI証拠は、manifest／version／identity／sessionとの対応をfresh Evaluatorが確認した場合に再利用できる。cacheを直接編集せず、Claude marketplaceのlegacy-compatible install、手動 `.agents/skills`／`AGENTS.md`／`config.toml`、manifest／marketplace欠落、誤path、同名skill重複、1 Codex hostだけのPASSを正式配布PASSにしない。
44. **4ホストの実用出荷受入**: private `origin/main` の `4670438` が評価candidateであることを確認し、Claude Code Desktop App／CLI、Codex App／CLIの4件を別々に評価する。必須証拠は、正式manifest／marketplace、`0.8.0`、Agentic identity、正本skillの一意読込、current-bytes実会話8面のMarkdown、Chatwork／Google Chat wizard、workspace変更0件、Secret露出0件、更新／再導入手順、全回帰・archive・利用可能な公式validatorである。GUI Appは実UI、host-owned session record、AX tree、screenshot等、CLIは実command／session evidenceでよく、同一attestation schemaを要求しない。既取得の4 host固有証拠は、commit／version／identity／skill／sessionがcandidateと一致する場合に再利用し、欠落または不一致がある面だけ軽量に再確認する。実会話runnerを新たに自動実行する場合は、合成HOME、read-only plugin copy、env allowlist、最小tool、sandbox／path-scoped permission、workspace外canary拒否、cleanupの安全契約を維持する。schema v2／v3 collector／driver／attestorや12×8 exact resultの有無は採点しない。
...
50. **明示memory authorizationとYasashii下流独立評価**: 固定base `3c472dd9a2b5299f27741ae2c418094486b7d035`、公開製品commit `9acea13477cd7730bf064a32c170b752586fa116`、公開candidate ID `36a5c5f5482fcd510e5b361bdf9e24620be696046e248fb29b3b557800cc083d`、Yasashii隔離product candidate ID `4bc87169d87baf90f9681f7ba07d3154c71df34eac78bad15b435732e876faf2`、schema 3 handoff digest `e515842b147393ac77dddfb94d000188916d4aa837fda17d7e8fb4015f844982` を固定する。manifestと同じbuilder runからparity／adapted／supporting、declared union、read／copy／write／execute／protect、actual diffを機械導出し、role重複、未利用宣言、stale path、未分類mutationを0件にする。parityのbyte／mode、adaptedのtransformer／anchor出現・実適用・final digest、supportingの実利用、copy後adapt trace、`publicWholeTree`と各負fixture、overlay record／apply／check／reapply二回目差分0件、Yasashii固有surface不変を確認する。観測件数の固定値だけではPASSにしない。実Yasashii sourceで専用15/15、Sprint 038／010／safety、release integrity、Git-free archive、conversation core inventory 17/17を実行し、実runtime goldenでrun-once、request／content hedge、pending、append-only訂正、内容dedupe、checkpoint partial、memory scope gate、既存6操作を確認する。公開PASSをYasashii PASSへ流用せず、UI変更と外部writeが0件であることを記録する。
...
| 033 | `4670438` 完成候補、共通祖先、agentic技術表現、共通本体＋host adapter、Claude `.claude-plugin`、Codex `.codex-plugin/plugin.json`＋repo marketplace、4 host固有のcurrent-bytes実機smoke、0.8.0／identity／15 skill、会話8面Markdown、wizard、workspace変更0件、Secret露出0件、更新／再導入手順、全回帰・archive・validator、既取得証拠の整合確認、schema v2／v3証明基盤のNon-scope化、public／release／OAuth／Secret／実API禁止 |
...
- `verification-scope-issue`: 製品ではなく検証基盤の過剰要件・不備だけが原因。Generator／Plannerへ自動で戻さず、現契約のsafe harbor、未達、選択肢を示してユーザーへ返す。
...
- 2026-07-21: OpenAI公式のplugin／skill仕様とCodex CLI 0.144.6の実機確認を受け、Sprint 033のCodex配布面を手動 `config.toml`／`AGENTS.md`／skillsコピーから正式 `.codex-plugin/plugin.json`＋marketplaceへ改訂。検証方法43とC15を追加し、GitHub marketplace、Plugins Directory／CLI browser、新規chat／session、15 skill一意discover、明示／暗黙trigger、更新・cache、legacy互換非代替を必須化した。
