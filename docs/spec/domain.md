...
### 検証済みprivate workspaceとの同等性

既存private workspaceで検証済みのGoogle Chat同期を振る舞いの基準にする。同等とみなす要素は次である。
...
- 既存workspace固有の自動取得間隔。本製品はChatworkと揃え、3時間を推奨・初期値にする。
...
候補はaccount-name経路でだけ一時的に作る。`value`、`source`、最上位1件の `recommended` を持ち、
sourceは現在会話、Personalization、Project、hostから現在タスクへ渡された記憶、Git表示名、OSユーザー名の順とする。
NFKC＋Unicode case-foldで同値の候補は上位sourceの1件へ統合する。候補0件は正常な
`unavailable` 状態であり、架空値を作らず別経路へ戻す。

探索値、除外値、出典一覧、順位はworkspaceへ保存しない。確認済みの現在値は
`secretary/memory/preferences.md` を正本とし、`secretary/AGENTS.md` と
`secretary/memory/MEMORY.md` の現役表示を同じtransactionで同期する。
初回decisionは当時の判断履歴であり、現在値へ追随させない。

transactionは3正本とjournalの開始snapshotを持ち、file書込み、journal、commitの失敗では
全対象を開始時へ戻す。成功時だけjournalと所有path限定local commitを各1件作り、pushしない。
...
- downstream checkout: `<workspace-root>/yasashii-secretary`
