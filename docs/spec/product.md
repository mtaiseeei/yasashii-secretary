...
既存private workspaceで検証済みのGoogle Chat同期を基準に、スペース別・日付別Markdown、スレッド、発言者、Asia/Tokyoの時刻、
初回の取得可能な全履歴、以後の差分取得を保つ。ただし、使っていない権限、古いサービスアカウント案内、
資格情報を端末へ表示する挙動は引き継がない。

自動取得の既定推奨は3時間ごとにする。利用者は手動のみ、1時間、3時間、6時間、12時間から選べ、
確定前に対象、保存内容、共同編集者への可視性、commit・pushを確認する。保存形式と取得境界は検証済みの共通契約を基準にするが、
...
Claude系は既存の `.claude-plugin` manifest／marketplace、Codex系は
`.codex-plugin/plugin.json` と `.agents/plugins/marketplace.json` を正式な共有配布面とする。
両manifestは同じ `plugins/secretary/skills/` を参照し、skill本文をホスト別に複製しない。
Codexのrepo-local skillsや手動の `AGENTS.md`／`config.toml` 設定はauthoring・test補助または
fallbackに限り、正式Codex pluginを欠いた状態を配布完成とは扱わない。

### G12 呼び方と配布物を利用者中立にする

Claude CodeとCodexの初回セットアップは、同じ4つの呼び方経路と保存前確認を提供する。
「アカウント名」を選んだ場合だけ、現在タスクへhostが提供済みの文脈、Git表示名、OSユーザー名を順に確認し、
名前らしい候補へ整えて出典つきで提案する。任意の過去会話や生session logを直接探索せず、
不適格な値は候補にせず、候補が無ければ他の経路へ戻れる。

既存workspaceの呼び方変更では、確認済み値だけを `preferences.md`、`AGENTS.md`、`MEMORY.md` の
現役表示へ同期し、初回決定ログを履歴として残す。探索結果は保存しない。

Agenticの共通機能は、独立評価PASS済みの完全SHAをreview済みbaseとしてYasashii overlayへ同期する。
Yasashii固有の言葉遣い・identity・配布metadata・repo-owned正本は維持する。
...
- 呼び方は「あなた」「アカウント名」「指定の名前」「その他」から選べる。「アカウント名」の候補はhost提供済み文脈→Git→OSの順で適格値だけを示し、探索結果ではなく確認済み値だけを保存する。
...
- `4670438` について、4 host固有の実機smoke、正式manifest／marketplace、`0.8.0`、Agentic identity、正本skill、会話8面のMarkdown、wizard、workspace変更0件、Secret露出0件、更新／再導入手順、全回帰・archive・利用可能な公式validatorが整合する。GUI Appは実UI証拠、CLIはcommand／session証拠でよく、同一の証明基盤を要求しない。
...
- `<workspace-root>/agentic-harness` を操作しない。編集だけでなくcheckout、commit、branch、remote変更、生成物作成、複製元利用、コマンド対象化を禁止し、上流参照はGitHubに限定する。
...
- Sprint 033の製品scopeとしてschema v2／v3のproduction collector／driver／attestor、期限つきapproval、challenge、二層artifact、12×8 exact resultを作らない。これらは将来のoptional internal QAであり、配布Pluginへ同梱必須にしない。
...
- 2026-08-26、公開Agentic Sprint 040の独立PASS済み製品commit `09267e352db51227e3f1375d861df53139797249` を入力とし、固定base `3c472dd9a2b5299f27741ae2c418094486b7d035` から宣言済みcommon／adapted pathだけをYasashiiへ適用することを承認した。公開candidate ID `428b3ff435ee63bf47837e38792873264e14336e85ca1190bd823e80cbc67e0a` とYasashii隔離product candidate ID `bb194d55a3cff4fe6fbfdb588f1db665d4fcd2ed4446482410ca9dc525490cfd` を固定し、Yasashii固有surface保護と独立評価を必須とする。
