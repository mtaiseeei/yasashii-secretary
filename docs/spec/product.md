...
- marketplace、plugin manifest、CHANGELOG、更新台帳、migrationが `0.7.0`で整合し、既存 `0.6.0`からのdry-run／更新／再実行／失敗復元が成立する。rollbackはworkspaceとpluginを区別し、両方を更新前状態へ戻すか、その場で実行できる正確な復元手順を示す。
