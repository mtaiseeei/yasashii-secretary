...
記憶・成果物・通常のprojectを1つのprivate GitHub repoに置き、選択したChatwork roomの履歴も同じrepoで管理します。
...
考え方はシンプルです。**メールや予定は各サービスに置いたまま参照し、秘書・project・選択したChatwork履歴は1つのprivate repoで育てる**。
Chatworkの定期pushは、対象room・頻度・保存内容を確認して同意した後だけ有効になります。
...
1. `/chatwork` を実行し、private repoと接続状態を確認します。
2. 案内された `gh secret set CHATWORK_API_TOKEN` でAPI TokenをRepository Secretへ登録します。Token値はrepoや画面へ保存しません。
3. room一覧を取得し、ローカルwizardで保存するroomを選びます。初回は各roomの最新100件以内で、0件も正常です。
4. 30分／1時間／3時間／6時間／12時間／手動のみから頻度を選びます。月間run数は概算で、GitHub Actionsの実課金分数とは異なります。
5. 対象room・頻度・保存内容・自動commit／pushを確認し、同意後だけscheduleを有効にします。

`/chatwork search` は最初にrepoをpullして保存済み履歴を探します。見つからないときは
「同期して再検索（推奨）／同期しない／対象roomを見直す」から選び、承認した場合だけ手動同期します。
同期後も、導入前・最新100件より前・未選択room等の可能性があるため、「Chatworkに存在しない」とは断定しません。
...
| 秘書の記憶（決定・好み・進行中案件） | private repo内 `secretary/memory/` | 直接読み書き＋Git管理 |
| 成果物（文書の正本） | private repo内 `secretary/docs/` | 直接読み書き＋Git管理 |
| 選択したChatwork room履歴 | 同じprivate repo内 `chatwork/` | Repository Secretを使うGitHub Actions |

- 外部データのローカル同期層（キャッシュ・全文コピー）は作りません。根拠は「サービス名＋リンク/ID＋日付」で引用します。
- Chatwork scheduleの自動commit・pushは、対象room・頻度・保存内容への明示同意後だけです。検索時の手動同期も実行直前に確認します。
