...
`/google-chat` から、選択した通常スペースだけを同じ非公開のGitHubリポジトリへ保存できます。
この設定には、**Google Workspace管理者、またはGoogle Cloudプロジェクトを作成できる方**が必要です。

ご自身が管理者、またはGoogle Cloudプロジェクトを作成できる場合は、次の順で設定します。別の方が管理する場合は、この5項目をその方へ依頼してください。

1. [会社のGoogle Workspace組織が所有するGoogle Cloudプロジェクトを作る](https://docs.cloud.google.com/resource-manager/docs/creating-managing-projects)。
2. [Google Chat API](https://console.cloud.google.com/apis/library/chat.googleapis.com)と[Google People API](https://console.cloud.google.com/apis/library/people.googleapis.com)を有効にする。
3. [Google Auth platform](https://developers.google.com/workspace/guides/configure-oauth-consent)で、Audienceを `Internal` にする。
4. OAuth Clientを `Desktop app` として作る。
5. 作成した接続用ファイルをダウンロードする。

![Google Cloudで接続用ファイルを準備する5ステップの画面例](plugins/yasashii-secretary/skills/google-chat/assets/wizard/google-cloud-setup-guide.svg)

> この画像はアカウント情報を省略した画面例です。メールアドレス、アカウント名、client ID、client secret、プロジェクト固有の識別値は含みません。Google側UIは2026年7月確認で、今後変わる可能性があります。
...
公式情報は2026年7月確認です。Google側の画面、scope分類、管理者設定は変更される可能性があります。詳しくは[ユーザーとしてGoogle Chat APIを認証する](https://developers.google.com/workspace/chat/authenticate-authorize-chat-user)と[Desktop appのloopback OAuth](https://developers.google.com/identity/protocols/oauth2/native-app)を確認してください。
