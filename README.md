...
Google Workspace版Google Chatに対応しています。AIへ **「Google Chatを設定したい」** と伝えてください。
現在のGitリポジトリ名からGoogle CloudのProject案を作り、Google公式CLIの `gcloud` で進められるところまで一緒に準備します。
Google画面で本人の操作が必要なときは、対象Projectを指定した公式リンクと、今押す場所を一つずつ案内します。
