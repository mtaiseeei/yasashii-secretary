...
**非エンジニア向けのAI秘書プラグイン**（Claude Code / Codex plugin、public、MIT）。
...
Claude Codeでは、上から順に実行します。各コマンドの前に「今から何をするか」を書いています。
...
```text
# 1. このプラグインの配布元を登録する
codex plugin marketplace add mtaiseeei/yasashii-secretary

# 2. yasashii-secretary プラグインを入れる
codex plugin add yasashii-secretary@yasashii-secretary

# 3. 新しいchat／sessionで秘書を呼ぶ
$secretary
```
...
├── .agents/plugins/marketplace.json  ← Codex用の正式repository marketplace
├── plugins/secretary/        ← プラグイン本体（薄いルーター＋機能スキル）
│   ├── .claude-plugin/・.codex-plugin/ ← host別の正式manifest
...
│   ├── templates/                    ← 秘書ディレクトリの雛形（`${SECRETARY_PLUGIN_ROOT}` 相対で参照）
...
- 各skillは、読み込まれた `SKILL.md` 自身の実ファイル絶対pathからplugin rootを解決します。空path、相対path、未解決placeholder、現在directoryからの推測はcommandへ渡しません。
...
Codexのもう1つの明示入口は `$harness-loop` です。通常はどちらのhostでも「〇〇を作って」と話すだけで開始できます。
