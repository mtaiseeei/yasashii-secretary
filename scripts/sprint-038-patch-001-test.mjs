...
check("0.9.1 history remains and current release surfaces are exactly 0.9.2", () => {
  assert.equal(json(join(root, ".claude-plugin/marketplace.json")).plugins[0].version, "0.9.2");
  assert.equal(json(join(plugin, ".claude-plugin/plugin.json")).version, "0.9.2");
  assert.equal(json(join(plugin, ".codex-plugin/plugin.json")).version, "0.9.2");
