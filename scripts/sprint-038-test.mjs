...
check("current release owners are 0.10.3 and historical 0.10.1／0.8 migration remain unchanged", () => {
  assert.equal(JSON.parse(readFileSync(join(root, ".claude-plugin/marketplace.json"))).plugins[0].version, "0.10.3");
  assert.equal(JSON.parse(readFileSync(join(root, "plugins/secretary/.claude-plugin/plugin.json"))).version, "0.10.3");
  assert.ok(JSON.parse(readFileSync(join(root, "plugins/secretary/.codex-plugin/plugin.json"))).version.startsWith("0.10.3"));
