...
check("fixed upstream base, release candidate and complete tree are recorded", () => {
  ...
  assert.equal(base.releaseCandidate, "0.10.3");
...
check("Yasashii identity, copy, Harness route, rule graph and 0.10.3 release surfaces are intact", () => {
  ...
  assert.equal(claudePlugin.version, "0.10.3");
  assert.equal(codexPlugin.name, "yasashii-secretary");
  assert.equal(codexPlugin.version, "0.10.3");
