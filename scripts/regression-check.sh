...
#   5. 非エンジニア体験（既定3項目＋明示「くわしく」だけ補足1つ・不変規律）
...
if plugins:
    ...
    if p.get("source") != "./plugins/secretary":
...
STYLE_RULES="$PLUGIN/rules/styles/yasashii.md"
EDITION_COPY="$PLUGIN/rules/copy/yasashii.json"
...
check "rule manifest・分離rule・4面copyが存在" \
  "[ -f '$RULE_MANIFEST' ] && [ -f '$SAFETY_RULES' ] && [ -f '$EVIDENCE_RULES' ] && [ -f '$COMMON_RULES' ] && [ -f '$STYLE_RULES' ] && [ -f '$EDITION_COPY' ]"
...
communication_contract_ok() {
  local entry="$1" style="$2" common="$3" safety="$4" evidence="$5" agents="$6" claude="$7"
  grep -q 'rule-manifest.json' "$entry" &&
    grep -q 'styles/yasashii.md' "$entry" &&
    grep -q '最終応答serializer（通常報告の唯一の正本）' "$style" &&
    grep -q 'tool実行を無言で完了' "$style" &&
    grep -q '1回だけ適用' "$style" &&
    grep -q '明示的に「くわしく」の場合だけ' "$style" &&
    grep -q 'Markdown箇条書きとして物理的に分けます' "$style" &&
    ...
    ! grep -qE '口調.*報告.*補足|専門用語.*報告.*補足|役割.*報告.*補足' "$style" "$agents" "$claude"
}
...
check "既定3項目Markdown＋明示くわしくのみ補足1つはrules唯一正本＋2面参照" \
  "communication_contract_ok '$RULES' '$STYLE_RULES' '$COMMON_RULES' '$SAFETY_RULES' '$EVIDENCE_RULES' '$PLUGIN/templates/AGENTS.md' '$PLUGIN/templates/CLAUDE.md'"
check "共通表現に進行語彙（計画→道具→確認→結果）" "grep -q '計画' '$COMMON_RULES' && grep -q '道具' '$COMMON_RULES' && grep -q '確認' '$COMMON_RULES' && grep -q '結果' '$COMMON_RULES'"
check "証拠／共通表現に英語エラー翻訳の方針" "grep -q '英語エラー' '$EVIDENCE_RULES' && grep -q '英語エラー' '$COMMON_RULES'"
check "onboarding は完了内容に次の操作を含めserializerへ渡す" \
  "grep -q '次に試せる操作' '$ONBOARD_SKILL' && grep -q '通常報告の項目数、prefix、Markdown構造、完成例は持たず' '$ONBOARD_SKILL'"
...
check "意図的失敗fixture: 口調による暗黙の報告長変更を検出" \
  "! communication_contract_ok '$RULES' '$STYLE_RULES' '$COMMON_RULES' '$SAFETY_RULES' '$EVIDENCE_RULES' '$WORK/AGENTS-invalid-report-override.md' '$PLUGIN/templates/CLAUDE.md'"
...
check "daily は内容だけを返しserializerへ出力形を委譲" \
  "grep -q 'ユーザーが選べる次の行動' '$DAILY_SKILL' && grep -q '通常報告の項目数、prefix、Markdown構造、前後の包装は定義せず' '$DAILY_SKILL'"
...
# ---------------------------------------------------------------------------
# 配布物全体を対象に検査する: プラグイン本体（plugins/secretary・templates 含む）に加え、
...
# (3) 分離rule群が改訂 ui.md の不変規律＋許可上書きを反映
check "共通表現が『そのまま使う語』方針を明記" "grep -q 'そのまま使う語' '$COMMON_RULES'"
check "共通表現が『初出のみ補足』方針を明記" "grep -q '初出時' '$COMMON_RULES'"
check "共通表現が幼稚メタファー禁止を明記" "grep -q '幼稚なメタファー' '$COMMON_RULES' && grep -q '秘書ディレクトリ' '$COMMON_RULES'"
check "共通表現が過度な平易化をしない旨を明記" "grep -q '過度な平易化' '$COMMON_RULES'"
check "entrypointが安全・証拠・共通表現・styleを分離" \
  "grep -q 'safety.md' '$RULES' && grep -q 'evidence.md' '$RULES' && grep -q 'common-language.md' '$RULES' && grep -q 'styles/yasashii.md' '$RULES'"
check "ことば添えでも一般技術用語を置換しない" \
  "grep -q 'ことば添え.*一般技術用語を別の語へ置換せず' '$STYLE_RULES'"
check "そのままOKでも安全説明を省かない" \
  "grep -q 'そのままOK.*安全説明を省きません' '$SAFETY_RULES'"
check "報告の項目数はくわしく以外の設定で変わらない" \
  "grep -q '口調・専門用語・役割は項目数を変えません' '$STYLE_RULES' && ! grep -qE '口調.*報告.*補足|専門用語.*報告.*補足|役割.*報告.*補足' '$STYLE_RULES' '$PLUGIN/templates/AGENTS.md' '$PLUGIN/templates/CLAUDE.md'"
...
check "共通表現にそのまま使う語リスト（ディレクトリ・コミット等）" "grep -q 'ディレクトリ' '$COMMON_RULES' && grep -q 'コミット' '$COMMON_RULES' && grep -q 'コネクタ' '$COMMON_RULES'"
...
check "connections は診断内容だけを返しserializerへ出力形を委譲" \
  "grep -q 'ユーザーが選べる次の接続案内' '$CONNECTIONS_SKILL' && grep -q '通常報告の項目数、prefix、Markdown構造、完成例は持たず' '$CONNECTIONS_SKILL'"
...
# ---------------------------------------------------------------------------
section "23. 確認後だけ行う安全な更新（sprint-018）"
# ---------------------------------------------------------------------------
SPRINT018_REGRESSION="$REPO/scripts/sprint-018-regression.sh"
check "sprint-018回帰が存在し実行可能" "[ -x '$SPRINT018_REGRESSION' ]"
if bash "$SPRINT018_REGRESSION"; then
  ok "sprint-018保護commit・更新・migration・rollbackが全て成功"
else
  ng "sprint-018回帰に失敗"
fi

# ---------------------------------------------------------------------------
section "24. Google Chat高度接続・初回取得（sprint-019）"
# ---------------------------------------------------------------------------
SPRINT019_REGRESSION="$REPO/scripts/sprint-019-regression.sh"
check "sprint-019回帰が存在" "[ -f '$SPRINT019_REGRESSION' ]"
if bash "$SPRINT019_REGRESSION"; then
...
# ---------------------------------------------------------------------------
section "32. edition rule境界と4面copy（sprint-029）"
# ---------------------------------------------------------------------------
SPRINT029_REGRESSION="$REPO/scripts/sprint-029-regression.sh"
check "sprint-029回帰が存在し実行可能" "[ -x '$SPRINT029_REGRESSION' ]"
if bash "$SPRINT029_REGRESSION"; then
  ok "sprint-029 rule graph・4面copy・安全override・wizard不変が全て成功"
else
  ng "sprint-029回帰に失敗"
fi
...
check "既存CLAUDE.mdの製品固有境界・secret・3項目報告を維持" \
...
check "candidate version 0.8.0とHarness/agents非同梱を維持" \
  "python3 -c \"import json; assert json.load(open('$MARKET'))['plugins'][0]['version'] == '0.8.0'; assert json.load(open('$PLUGINJSON'))['version'] == '0.8.0'\" && [ ! -d '$PLUGIN/harness' ] && [ ! -d '$PLUGIN/agents' ]"
