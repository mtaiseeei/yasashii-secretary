...
const bootstrap = await (await fetch(`${wizardUrl}api/bootstrap`)).json();
...
const beforeDiscovery = await fetch(`${wizardUrl}api/confirm`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ selectedRoomIds: ["101"], interval: "manual", automaticPushConsent: false }) });
...
const discovered = await fetch(`${wizardUrl}api/discover`, { method: "POST" });
