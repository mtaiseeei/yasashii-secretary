...
try {
  ...
  check("current marketplace/pluginは0.10.3で一致", market.plugins?.[0]?.version === "0.10.3" && pluginManifest.version === "0.10.3");
  check("canonical/legacy CHANGELOGは0.10.3先頭かつbyte一致", readFileSync(candidateChangelog).equals(readFileSync(legacyChangelog)) && readFileSync(candidateChangelog, "utf8").startsWith("# 変更履歴\n\n## [0.10.3]"));
  check("editionのcandidate/latest参照はyasashii 0.10.3配布面と整合", edition.edition === "yasashii-secretary" && edition.distribution.marketplaceUrl.includes("yasashii-secretary") && edition.distribution.changelogUrl.endsWith("/plugins/yasashii-secretary/CHANGELOG.md"));
  ...
  check("新規0.10.3 onboardingはneutral markerを作る", fresh.prepared.status === 0 && marker.schemaVersion === 1 && marker.edition === "yasashii-secretary", fresh.prepared.stderr);
  check("新規0.10.3導入はedition付きledgerを作る", fresh.ledger.status === 0 && ledger.schemaVersion === 2 && ledger.edition === "yasashii-secretary" && ledger.records?.length === fresh.managed.length && ledger.records.every((record) => record.installedVersion === "0.10.3"), `${fresh.ledger.stderr}${fresh.ledger.stdout}`);
  check("新規0.10.3配布物は主要skillと両wizardを含む", [
  ...
  check("0.10.3→0.10.3はCTAなし・副作用0", same.status === 0 && parse(same).status === "same" && parse(same).choices?.find((item) => item.id === "proceed-update")?.available === false && sameStart.status === 0 && JSON.stringify(workspaceSnapshot(fresh.workspace)) === JSON.stringify(sameBefore), `${same.stderr}${same.stdout}${sameStart.stderr}${sameStart.stdout}`);
