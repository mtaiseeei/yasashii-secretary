...
check "templates/tones/全14スキルは正本参照だけでschema重複0" \
  "[ '${#REFERENCE_SURFACES[@]}' -eq 19 ] && [ '$reference_bad' -eq 0 ]"
...
check "全14スキルがpreferencesを参照" "[ '$missing_pref_ref' -eq 0 ] && [ \"\$(find '$PLUGIN/skills' -mindepth 2 -maxdepth 2 -name SKILL.md | wc -l | tr -d ' ')\" -eq 14 ]"
