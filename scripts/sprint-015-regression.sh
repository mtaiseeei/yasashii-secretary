...
new_sec(){
  ...
  mkdir -p "$sec/memory/decisions" "$sec/memory/journal" "$sec/memory/topics" "$sec/projects/open" "$sec/inbox" "$sec/docs"
...
check_eval "作成確認前はexit 3・副作用0件" "[ '$RC' -eq 3 ] && [ '$BEFORE' = '$AFTER' ] && [ ! -e '$SEC/projects/open/営業改善' ]"
check_eval "候補拒否は実行コマンド不要で副作用0件" "[ '$(count_journal "$SEC")' -eq 0 ] && [ -z \"\$(find '$SEC/projects/open' -mindepth 1 -print -quit)\" ]"
...
OUTSIDE="$WORK/outside"; mkdir -p "$OUTSIDE"; ln -s "$OUTSIDE" "$SEC/projects/open/外向き"
...
rm -f "$SEC/projects/open/外向き"

# ライト作成3業務・同名保護・journal
create "$SEC" 営業改善
create "$SEC" Instagramマーケティング
create "$SEC" 新規事業
check_eval "営業ライトPJは実内容とstatus active" "grep -q '^status: active$' '$SEC/projects/open/営業改善/PROJECT.md' && grep -q '営業改善の概要' '$SEC/projects/open/営業改善/PROJECT.md' && grep -q '次の入口' '$SEC/projects/open/営業改善/PROJECT.md'"
check_eval "マーケティング・新規事業fixtureを作成" "[ -f '$SEC/projects/open/Instagramマーケティング/PROJECT.md' ] && [ -f '$SEC/projects/open/新規事業/PROJECT.md' ]"
ORIGINAL="$(shasum "$SEC/projects/open/営業改善/PROJECT.md" | awk '{print $1}')"
p create-light "$SEC" 営業改善 --overview 上書き --goal ゴール --success 成功 --current 現在 --next 次 --confirm >/dev/null 2>&1
check_eval "同名PJは拒否し既存内容を保護" "[ $? -eq 3 ] && [ '$ORIGINAL' = \"\$(shasum '$SEC/projects/open/営業改善/PROJECT.md' | awk '{print \$1}')\" ]"
check_eval "ライト作成成功だけjournalへ各1件" "[ \"\$(grep -Rh 'ライト運用で作成' '$SEC/memory/journal' | wc -l | tr -d ' ')\" -eq 3 ]"
ln -s "$OUTSIDE" "$SEC/projects/open/営業改善/危険な参照"
PJ_BEFORE="$(shasum "$SEC/projects/open/営業改善/PROJECT.md" | awk '{print $1}')"
p add-note "$SEC" 営業改善 --note '保存してはいけない' --confirm >/dev/null 2>&1
check_eval "既存PJ内symlinkがあれば更新を拒否" "[ $? -eq 3 ] && [ '$PJ_BEFORE' = \"\$(shasum '$SEC/projects/open/営業改善/PROJECT.md' | awk '{print \$1}')\" ] && [ -z \"\$(find '$OUTSIDE' -mindepth 1 -print -quit)\" ]"
rm -f "$SEC/projects/open/営業改善/危険な参照"

# 決定・状態・TODO・memory重複境界
p add-decision "$SEC" 営業改善 --decision '対象業種は製造業にする' --current '製造業の候補を抽出中' --next '候補10社を選ぶ' >/dev/null 2>&1
check_eval "決定確認前はPROJECT不変" "[ '$ORIGINAL' = \"\$(shasum '$SEC/projects/open/営業改善/PROJECT.md' | awk '{print \$1}')\" ]"
p add-decision "$SEC" 営業改善 --decision '対象業種は製造業にする' --current '製造業の候補を抽出中' --next '候補10社を選ぶ' --confirm >/dev/null
check_eval "確認済み決定と状態を同時更新" "grep -q 'D-001 (2026-07-17): 対象業種は製造業にする' '$SEC/projects/open/営業改善/PROJECT.md' && grep -q '製造業の候補を抽出中' '$SEC/projects/open/営業改善/PROJECT.md' && grep -q '候補10社を選ぶ' '$SEC/projects/open/営業改善/PROJECT.md'"
...
check_eval "PJ参照つきTODOは既存正本へ入る" "grep -q '候補10社を選ぶ.*PJ: 営業改善 / projects/open/営業改善/PROJECT.md' '$SEC/inbox/todo.md'"
check_eval "PJ内に生きたTODO.mdを作らない" "[ ! -e '$SEC/projects/open/営業改善/TODO.md' ]"

# 成果物境界とフル索引
printf '候補企業の作業中一覧\n' | p save-work "$SEC" 営業改善 --title 候補企業一覧 --tags 営業 >/dev/null
printf '商談方針の確定版\n' | p save-output "$SEC" 営業改善 --title 商談方針 --tags 営業,確定 >/dev/null
check_eval "PJ作業文書は直下・確定版はoutputs" "[ -f '$SEC/projects/open/営業改善/2026-07-17_候補企業一覧.md' ] && [ -f '$SEC/projects/open/営業改善/outputs/2026-07-17_商談方針.md' ]"
p archive-file "$SEC" 営業改善 2026-07-17_候補企業一覧.md >/dev/null 2>&1
check "archive確認前は元ファイルを保持" test -f "$SEC/projects/open/営業改善/2026-07-17_候補企業一覧.md"
p archive-file "$SEC" 営業改善 2026-07-17_候補企業一覧.md --confirm >/dev/null
check_eval "確認後だけ旧版をarchive" "[ ! -e '$SEC/projects/open/営業改善/2026-07-17_候補企業一覧.md' ] && [ -f '$SEC/projects/open/営業改善/archive/2026-07-17_候補企業一覧.md' ]"
...
for i in $(seq -w 1 11); do printf '# 文書%s\n' "$i" > "$FILES/projects/open/文書多数/2026-07-17_文書${i}.md"; done
...
check_eval "了承後だけ5ファイル構成" "for f in AGENTS.md PROJECT.md DECISIONS.md MEMORY.md CLAUDE.md; do [ -f '$DEC/projects/open/判断多数/'\"\$f\" ] || exit 1; done"
check_eval "フルPJはINDEX/TODOなし・CLAUDEポインタ" "[ ! -e '$DEC/projects/open/判断多数/INDEX.md' ] && [ ! -e '$DEC/projects/open/判断多数/TODO.md' ] && grep -q 'AGENTS.md' '$DEC/projects/open/判断多数/CLAUDE.md'"
check_eval "Decisions移行は欠落なし・PROJECTにサマリー保持" "[ \"\$(grep -c '^## D-' '$DEC/projects/open/判断多数/DECISIONS.md')\" -eq 11 ] && [ \"\$(grep -c '^- D-' '$DEC/projects/open/判断多数/PROJECT.md')\" -eq 11 ]"
check_eval "AGENTS Start hereと索引が実pathに一致" "grep -q '^## Start here' '$DEC/projects/open/判断多数/AGENTS.md' && for f in PROJECT.md DECISIONS.md MEMORY.md CLAUDE.md AGENTS.md; do grep -Fq \"\$f\" '$DEC/projects/open/判断多数/AGENTS.md' || exit 1; done"
...
check_eval "別repo確認前はpointer・remote変更0件" "[ $? -eq 3 ] && [ '$DEV_BEFORE' = \"\$(find '$DEV' -type f -exec shasum {} + | sort | shasum | awk '{print \$1}')\" ] && [ ! -e '$DEV/projects/open/顧客アプリ' ]"
...
check_eval "別repo workspace側はAGENTSとPROJECTだけ" "[ \"\$(find '$DEV/projects/open/顧客アプリ' -type f | wc -l | tr -d ' ')\" -eq 2 ] && [ -f '$DEV/projects/open/顧客アプリ/AGENTS.md' ] && [ -f '$DEV/projects/open/顧客アプリ/PROJECT.md' ]"
check_eval "別repo pointerに正本・公開範囲・最初に読むfile" "grep -q 'github.com/example/customer-app' '$DEV/projects/open/顧客アプリ/PROJECT.md' && grep -q '公開範囲: private' '$DEV/projects/open/顧客アプリ/PROJECT.md' && grep -q 'docs/spec.md' '$DEV/projects/open/顧客アプリ/AGENTS.md'"
check_eval "別repo pointerに仕様・判断・Sprint・成果物の複製なし" "[ ! -e '$DEV/projects/open/顧客アプリ/DECISIONS.md' ] && [ ! -e '$DEV/projects/open/顧客アプリ/MEMORY.md' ] && [ ! -e '$DEV/projects/open/顧客アプリ/docs' ] && [ ! -e '$DEV/projects/open/顧客アプリ/outputs' ]"

# 完了・再開・status欠落
COMP="$(new_sec complete)"; create "$COMP" 研修準備
H0="$(shasum "$COMP/projects/open/研修準備/PROJECT.md" | awk '{print $1}')"
p complete "$COMP" 研修準備 --result '研修を9回実施' --remaining 'アンケート集計' >/dev/null 2>&1
check_eval "完了確認前はstatus不変" "[ $? -eq 3 ] && [ '$H0' = \"\$(shasum '$COMP/projects/open/研修準備/PROJECT.md' | awk '{print \$1}')\" ]"
p complete "$COMP" 研修準備 --result '研修を9回実施' --remaining 'アンケート集計' --confirm >/dev/null
check_eval "完了後はclosedでcompleted・日付・結果・残件を保持" "[ ! -e '$COMP/projects/open/研修準備' ] && grep -q '^status: completed$' '$COMP/projects/closed/研修準備/PROJECT.md' && grep -q '2026-07-17: 結果=研修を9回実施 / 残件=アンケート集計' '$COMP/projects/closed/研修準備/PROJECT.md'"
CANDIDATE_BEFORE="$(find "$COMP" -type f -exec shasum {} + | sort | shasum | awk '{print $1}')"
p candidate-check "$COMP" 研修準備 --multiple-actions --repeated-topic --closed > "$WORK/completed-candidate.json"
...
check_eval "完了済みPJの候補照合は副作用0件" "[ '$CANDIDATE_BEFORE' = '$CANDIDATE_AFTER' ]"
 p list "$COMP" > "$WORK/active.txt"; p list "$COMP" --all > "$WORK/all.txt"; p list "$COMP" --include-closed > "$WORK/closed.txt"
check_eval "completedは通常一覧と--allから外れ明示一覧だけで見つかる" "! grep -q '研修準備' '$WORK/active.txt' && ! grep -q '研修準備' '$WORK/all.txt' && grep -q '研修準備 \[completed\]' '$WORK/closed.txt'"
p show "$COMP" 研修準備 --closed > "$WORK/show.txt"
check "completedはclosed明示参照で見つかる" grep -q '研修を9回実施' "$WORK/show.txt"
HC="$(shasum "$COMP/projects/closed/研修準備/PROJECT.md" | awk '{print $1}')"
p reopen "$COMP" 研修準備 --reason '追加研修の依頼' --next '日程候補を出す' >/dev/null 2>&1
check_eval "再開確認前はclosedのまま" "[ $? -eq 3 ] && [ '$HC' = \"\$(shasum '$COMP/projects/closed/研修準備/PROJECT.md' | awk '{print \$1}')\" ]"
p reopen "$COMP" 研修準備 --reason '追加研修の依頼' --next '日程候補を出す' --confirm >/dev/null
check_eval "確認後だけactiveへ戻り完了記録を保持" "grep -q '^status: active$' '$COMP/projects/open/研修準備/PROJECT.md' && grep -q '追加研修の依頼' '$COMP/projects/open/研修準備/PROJECT.md' && grep -q '研修を9回実施' '$COMP/projects/open/研修準備/PROJECT.md'"
...
sed -i.bak '/^status: active$/d' "$COMP/projects/open/研修準備/PROJECT.md"; rm -f "$COMP/projects/open/研修準備/PROJECT.md.bak"
p list "$COMP" > "$WORK/missing-status.txt"
check "status欠落はactiveとして一覧に出る" grep -q '研修準備 \[active\]' "$WORK/missing-status.txt"
check_eval "完了・再開はopen・closed間だけで移動" "[ -d '$COMP/projects/open/研修準備' ] && [ ! -e '$COMP/projects/closed/研修準備' ] && [ ! -e '$COMP/archive' ]"
...
check_eval "journal失敗時はproject部分生成をrollback" "[ $? -eq 3 ] && [ ! -e '$ROLL/projects/open/失敗PJ' ]"
CC_SECRETARY_NOW=2026-07-17T10:30 bash "$MEMORY" timeline "$SEC" --type journal --grep 営業改善 > "$WORK/timeline.txt"
check_eval "project名・要約・参照先からtimelineで再発見" "grep -q '営業改善' '$WORK/timeline.txt' && grep -q 'projects/open/営業改善/PROJECT.md' '$WORK/timeline.txt'"
p create-light "$SEC" 機密PJ --overview 'token=synthetic-secret-value' --goal ゴール --success 成功 --current 現在 --next 次 --confirm >/dev/null 2>&1
check_eval "資格情報らしき値を保存せず拒否" "[ $? -eq 3 ] && [ ! -e '$SEC/projects/open/機密PJ' ] && ! grep -Rql 'synthetic-secret-value' '$SEC'"
...
check_eval "ライトPJの全保存欄でGitHub PATを副作用なく拒否" "[ '$CRED_BEFORE' = '$CRED_AFTER' ] && [ -z \"\$(find '$CRED/projects/open' -mindepth 1 -print -quit)\" ] && ! grep -Rql '$PAT' '$CRED'"
p create-light "$CRED" FinePAT拒否 --overview "$FINE_PAT" --goal ゴール --success 成功 --current 現在 --next 次 --confirm >/dev/null 2>&1
check_eval "fine-grained GitHub PATも拒否" "[ $? -eq 3 ] && [ ! -e '$CRED/projects/open/FinePAT拒否' ] && ! grep -Rql '$FINE_PAT' '$CRED'"
p create-light "$CRED" URL拒否 --overview "$CRED_URL" --goal ゴール --success 成功 --current 現在 --next 次 --confirm >/dev/null 2>&1
check_eval "credential URLも拒否" "[ $? -eq 3 ] && [ ! -e '$CRED/projects/open/URL拒否' ] && ! grep -Rql 'synthetic-password' '$CRED'"
create "$CRED" 入力検査
...
check_eval "別repoポインタもGitHub PATを拒否" "[ $? -eq 3 ] && [ ! -e '$CRED/projects/open/開発資格情報' ]"
...
check_eval "通常のGitHub説明文は誤検知しない" "[ -f '$CRED/projects/open/GitHub案内/PROJECT.md' ] && grep -q 'GitHub PATはGitHub上の安全な保管場所へ保存する' '$CRED/projects/open/GitHub案内/PROJECT.md'"
